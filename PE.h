...
public:
  ...
  PIMAGE_OPTIONAL_HEADER GetOptionPEHeader() {
    return m_Option_Header;
  }                                                //可选PE头
  DWORD GetDosStubSize() { return m_DosStubSize; } // DOS残留垃圾数据大小
  DWORD ReadPE_File(__in char *lpszFile, __out LPVOID *pFileBuffer);
  DWORD CopyFileBufferToImageBuffer(__in LPVOID pFileBuffer,
                                    __out LPVOID *pImageBuffer);

  //功能函数
  BOOL RVAToFOA(DWORD RVA, DWORD &FOA, BOOL = FALSE);
  DWORD FeatureCodeMatch(std::vector<unsigned char> hexData,
                         std::vector<unsigned int> &MachAddress, BYTE *bMask,
                         char *szMask, DWORD AddressBase);
...
private:
  ...
  int CalculateFirstThunkData(DWORD FirstThunkBaseAddr,
