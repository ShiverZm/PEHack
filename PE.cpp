...
int  PE::CalculateThunkData(DWORD ThunkBaseAddr, ImportFuncInfo &importfunc_info)
{
	...
	while (true)
	{
		...
		//这里判断是否用的是序号方式
        DWORD addressData = pThunk->u1.AddressOfData;
        if ((addressData & 0x80000000) != 0)
		{
			//序号名方式
...
void PE::Anysis_Import_Table()
{
	...
	if (pImport_descriptorTmp->Name!=0)
	{
		while (true)
		{
			DWORD nameRva = pImport_descriptorTmp->Name;
			LPCSTR c_dllname = (char*)((unsigned char*)m_FileBuffer + (isMemoryLoad ? nameRva : tmp) + dwLen);
			...
			DWORD nameRva2 = pImport_descriptor->Name;
			std::string dllname = (char*)((unsigned char*)m_FileBuffer + (isMemoryLoad ? nameRva2 : foa));
